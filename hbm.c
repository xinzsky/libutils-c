...
static HBM_STRUCT * hbm_prepx(HBM_STRUCT *p, const unsigned char * pat, int m);
static HBM_STRUCT * hbm_prep(const unsigned char * pat, int m);
static unsigned char * hbm_match(HBM_STRUCT * px, const unsigned char * text, int n);
...
static unsigned char * hbm_match(HBM_STRUCT * px, const unsigned char * text, int n)
{
  ...
  {
    for (;t < et; t++)
      if (*t == *pat) return (unsigned char *)t;
    return NULL;
  }
                                            ...
                                            int no_case,int codetype)
{
  ...
  unsigned char text_l[TEXT_LOWER+1], pat_l[PAT_LOWER+1];
  unsigned char *text_p, *pat_p;
  int tdy, pdy;
  ...
  if (no_case)
  {
    if (n > TEXT_LOWER)
    {
      text_p = (unsigned char *)xcalloc(1, n + 1);
      tdy = 1;
    }
    else
    {
    ...
    *(text_p + n) = '\0';
    ...
    if (m > PAT_LOWER)
    {
      pat_p = (unsigned char *)xcalloc(1, m + 1);
      pdy = 1;
    }
    else
    {
    ...
    *(pat_p + m) = '\0';
    ...
    if (pdy)
      xfree(pat_p);
    if (puc)
      puc = (unsigned char *)(text + (puc - text_p));
